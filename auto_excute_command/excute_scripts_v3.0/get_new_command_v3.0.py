...
tester = "li, dongwangx"
command_file = rf"CXMT verification Status-20240711update.csv"
# ID	Title	Category	Priority	A:1DPC	Test Schedule	Test Result	Sighting	Test owner	Test Duration	Test Date	Test Config
schedule_file = rf"schedule_testcase.csv"
need_run_file = rf"testcase_running_command.csv"
...
def all_tcds():
    ...
    tcds_list =[]
    with open(schedule_file, 'r') as alltcds:
...
def have_cmd():
    # ID,Title,Cmd Line
    cmd_list = []
    with open(command_file, 'r') as ctcds:
...
def write_to_csv(cmd=get_new_tcd()):
    ...
    number =0
    with open(need_run_file, 'w', newline='') as newtcds:
        ...
        write_log("total add {0} tcds successfully\n".format(m))
...
def compare_ids(all=all_ids, have=have_ids):
    ...
    for i in range(len(all)):
        if all[i] in have:
            ...
            write_log(f"total {int(num)} tcds can't find ！")
