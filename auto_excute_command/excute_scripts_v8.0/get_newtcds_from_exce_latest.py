...
# 下面根据自己的机器情况进行修改
schedule_file = r"CXSH SPR4800 with DDR5-5600DIMM Dry Run Report - Copy" # schedule file 需要把后缀去掉，只保留文件名 eg: GNR-AP_BKC-Dryrun-24ww34.3.xlsx
sheet_name = r"Functional (1DPC)"  #  填写需要操作的sheet页的名字， 直接copy, 不要手输
host1 ="H0302"; host2 = "H0502"; # 如果只有一台，值要一致， 默认host = all
command_excel = r"CXDIMM-CMDline 3.xlsx"
sheet_command = r"CXDIMM-CMDline 2"
TESTER = "Yuan, WenliangX" #
            ...
            if schedule[numi][0] ==cxsh_com[numy][0]:# and schedule[numi][8] == TESTER :
                ...
                if schedule[numi][8] == TESTER:
                    if num %2 ==0:
                        ...
                        host = host2
                    num += 1
                df = pd.DataFrame({f'host':[host],'weekend':[schedule[numi][5]],'owner':[schedule[numi][8]],'category':[schedule[numi][2]],'priority':[schedule[numi][3]],'id':f'TC-{[schedule[numi][0]]}','title':[schedule[numi][1]],'cmd':[cxsh_com[numy][2]]})
