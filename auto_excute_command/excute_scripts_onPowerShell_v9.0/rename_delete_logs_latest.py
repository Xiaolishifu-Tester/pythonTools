...
# hostname = socket.gethostname().upper().split(".")
        ...
        if os.path.isdir(rf"{folder_path}\{file}"):
            ...
            for item in os.listdir():
                if os.path.isdir(item):
                    ...
                    os.chdir(logs_path)
                    if item.startswith("bios_serial_reader"):
                        pass
                    elif item.startswith(f"{hostnames[0]}_20") or item.startswith(f"{hostnames[1]}_20") or item.startswith(f"{hostnames[2]}_20"):
                        pass
                    else:
