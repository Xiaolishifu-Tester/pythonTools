...
"""
# Need to modify
schedule_file = r"CXSH-3200-64GB-Rev-B(2Rx4) (dry run)_schedule"  # 只需要复制原始表格的名称， 不要复制加了schedule字段的名称
ALLHOST = {"h193": "SHIPUNUC048"} # Keys:Values --> keys:host name  Values: host config
...
SAVE_RESULT_TO_EXCEL = True # True: 结果保存到 excel,  False: 结果不保存在excel
        ...
        if cmd:
            ...
            if ISPOWERSHELL == True:
                ...
                if str(cmd).strip(" ").startswith("python"):
                    exit_code = subprocess.run(["powershell.exe", cmd], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode #stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                else:
                    exit_code = subprocess.run(["powershell.exe", f"python {cmd}"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode
                if exit_code == 0:
                    print("===========test result: PASS ")
                else:
                    print("===========test result: FAIL ")
                print(f"============================end running, please see log , log in path: {autologpath} or {folder_path}")
            else:
                if str(cmd).strip(" ").startswith("python"):
                    exit_code = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode  # stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                else:
                    exit_code = subprocess.run( f"python {cmd}", stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode
            ...
            if SAVE_RESULT_TO_EXCEL == True:
                try:
                    df = pd.DataFrame({'TEST_TIME': [currenttimes], 'RESULT': [test_result],'ON_PowerShell': [str(ISPOWERSHELL)], 'ID': [id], 'HOSTNAME': [hostname[0]],'TITLE': [title],'LOG_FOLDER_NAME': [log_folder_name],'DURATION': [int(times+1)],'HOST_CONFIG':[host_key],'CATEGORY': [category], 'PRIORITY': [priority], 'CMD': [cmd]})
